...
import com.upokecenter.text.*;
 ...
 class EncodingGB18030 implements ICharacterReader, ICharacterEncoder {
  int gbk1, gbk2, gbk3;
  boolean gbk;
  DecoderState state;
 ...
 return 0x10000 + pointer-189000;
   ...
   private static int GBKIndex(int index) {
     throw new UnsupportedOperationException();
   }
  public EncodingGB18030 (ITransform stream, boolean gbk) {
   if ((stream) == null) {
  throw new NullPointerException("stream");
}
   this.state = new DecoderState(stream, 3);
  }
  public int ReadChar() {
    int c;
   while (true) {
 ...
 return -1;
       ...
       if (gbk3 != 0) {
         c=-1;
         if (b >= 0x30 && b <= 0x39) {
            int ap=(((gbk1-0x81)*10 + gbk2-0x30)* 126 + gbk3-0x81)*10 + b-0x30;
            c = GB18030CodePoint(ap);
         }
         if (c< 0) {
          state.PrependThree(gbk2, gbk3, b);
          gbk1 = gbk2 = gbk3 = 0;
          return 0xfffd;
         } else {
          gbk1 = gbk2 = gbk3 = 0;
