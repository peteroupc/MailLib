...
private HeaderFieldParsers() {
  ...
  HeaderEncoder enc = new HeaderEncoder(Message.MaxRecHeaderLineLength, name.length() +
          2);
        str = HeaderEncoder.TrimLeadingFWS(str);
        DowngradeHeaderFieldValue(enc, this, str);
      ...
      private static String DowngradeComments(
        ...
        for (int[] token : tokens) {
          ...
          if (token[0] == HeaderParserUtility.TokenComment) {
            ...
            enc.AppendString(str, lastIndex, endIndex);
          }
          lastIndex = endIndex;
      }
DebugUtility.Log("append ["
    ...
    @Override public int Parse(String str, int index, int endIndex, ITokener
      ...
      do {
        ...
        if (index < endIndex && (str.charAt(index) == 59)) {
          ...
          if (index == endIndex) {
    HeaderParserUtility.TraverseCFWSAndQuotedStrings(str, indexStart,
              endIndex,
              tokener);
          }
        }
