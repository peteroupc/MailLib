...
import com.upokecenter.text.*;
  ...
  public class CharsetsTest {
    static Object GetCharset(String name) {
      return Reflect.InvokeStatic(EncodingTest.MailNamespace() + ".Charsets", "GetCharset", name);
    }
    ...
    public void TestShiftJIS() {
      ...
      bytes = new byte[] {  (byte)0x82, 0x58, 0x33, 0x41, 0x61, 0x33, (byte)0x82, 0x60,
        ...
        (byte)0x82, 0x51  };
      String expected = "\uFF19\u0033\u0041\u0061\u0033\uFF21\uFF41\u0033\uFF71\uFF6F\u0033\u30A2\u30F6\u0033\u3042\u0033\u65E5\u0033\u003A\u003C\u0033\u00F7\u2103\u0033\u0031\uFF12\u0041\u0061\uFF12\uFF21\uFF41\uFF12\uFF71\uFF6F\uFF12\u30A2\u30F6\uFF12\u3042\uFF12\u65E5\uFF12\u003A\u003C\uFF12\u00F7\u2103\uFF12";
      Assert.assertEquals(expected, CharsetGetString(charset, EncodingTest.Transform(bytes)));
    }
    ...
    public void TestIso2022JP() {
      ...
      Assert.assertEquals(" Aa\\", CharsetGetString(charset, EncodingTest.Transform(bytes)));
      // Illegal byte in escape middle state
      bytes = new byte[] {  0x1b, 0x28, 0x47, 0x21, 0x41, 0x31, 0x5c  };
      Assert.assertEquals("\ufffd\u0028\u0047!A1\\", CharsetGetString(charset, EncodingTest.Transform(bytes)));
      // Katakana
      bytes = new byte[] {  0x1b, 0x28, 0x49, 0x21, 0x41, 0x31, 0x5c  };
      Assert.assertEquals("\uff61\uff81\uff71\uff9c", CharsetGetString(charset, EncodingTest.Transform(bytes)));
      bytes = new byte[] {  0x1b, 0x28, 0x49, 0x20, 0x41, 0x61, 0x5c  };
      Assert.assertEquals("\ufffd\uff81\ufffd\uff9c", CharsetGetString(charset, EncodingTest.Transform(bytes)));
      ...
      Assert.assertEquals("\u3000\u3001\u3002", CharsetGetString(charset, EncodingTest.Transform(bytes)));
      ...
      Assert.assertEquals("\ufffd\u0024\u004f!!!\u0023!#", CharsetGetString(charset, EncodingTest.Transform(bytes)));
      // JIS0212 state
      bytes = new byte[] {  0x1b, 0x24, 0x28, 0x44, 0x22, 0x2f, 0x22, 0x30, 0x22, 0x31  };
      Assert.assertEquals("\u02d8\u02c7\u00b8",
                      CharsetGetString(charset, EncodingTest.Transform(bytes)));
      bytes = new byte[] {  0x1b, 0x24, 0x28, 0x44, 0x22, 0x2f, 0x22, 0x30, 0x0a, 0x23, 0x31  };
      Assert.assertEquals("\u02d8\u02c7\n\u00231", CharsetGetString(charset, EncodingTest.Transform(bytes)));
      bytes = new byte[] {  0x1b, 0x24, 0x28, 0x44, 0x22, 0x2f, 0x22, 0x30, 0x1b, 0x28, 0x42, 0x23, 0x31  };
      Assert.assertEquals("\u02d8\u02c7\u00231", CharsetGetString(charset, EncodingTest.Transform(bytes)));
      bytes = new byte[] {  0x1b, 0x24, 0x28, 0x44, 0x22, 0x2f, 0x22, 0x0a, 0x22, 0x31, 0x23  };
      Assert.assertEquals("\u02d8\ufffd\u00b8\ufffd", CharsetGetString(charset, EncodingTest.Transform(bytes)));
      // Illegal state
      bytes = new byte[] {  0x1b, 0x24, 0x28, 0x4f, 0x21, 0x21, 0x21, 0x23, 0x21, 0x23  };
      Assert.assertEquals("\ufffd\u0024\u0028\u004f!!!\u0023!#", CharsetGetString(charset, EncodingTest.Transform(bytes)));
      // Illegal state at end
      bytes = new byte[] {  0x41, 0x1b  };
      Assert.assertEquals("A\ufffd", CharsetGetString(charset, EncodingTest.Transform(bytes)));
