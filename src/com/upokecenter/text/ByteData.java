...
import com.upokecenter.util.*;
  ...
  final class ByteData {
    ...
    public boolean GetBoolean(int cp) {
      if (cp < 0) {
        throw new IllegalArgumentException("cp (" + Long.toString((long)cp) + ") is less than " + "0");
      }
      if (cp > 0x10ffff) {
        throw new IllegalArgumentException("cp (" + Long.toString((long)cp) + ") is more than " + Long.toString((long)0x10ffff));
      }
