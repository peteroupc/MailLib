...
Assert.AreEqual(
  ...
  AssertGoodFilename("utf-8''%2A%EF%AB%87%EC%A5%B2%2B67%20Tqd%20R%E3%80%80%2E");
      ...
      Assert.AreEqual(
        ...
        stringTemp);
      {
        stringTemp = ContentDisposition.MakeFilename("=?utf=?utf-8?q?test?=");
        Assert.AreEqual(
          "=_utftest",
          stringTemp);
      }
      stringTemp = ContentDisposition.MakeFilename("=?utf-8?q=?utf-8?q?test?=");
      Assert.AreEqual(
        "=_utf-8_qtest",
        stringTemp);
      stringTemp =
  ...
  " =?utf-8*i-unknown?q?___hello.txt___?= ");
    ...
    ContentDisposition.MakeFilename("=?x-unknown?Q?file\ud800name?=");
     ...
     ContentDisposition.MakeFilename("=?utf-8?q?folder\\hello.txt?=");
      ...
      {
        stringTemp = ContentDisposition.MakeFilename("fol/der/hello.txt");
        Assert.AreEqual(
          "fol_der_hello.txt",
          stringTemp);
      }
